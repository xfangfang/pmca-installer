/**
 * 纯 JS SHA-256 与 HMAC-SHA256。
 *
 * 用到它的地方：xpd 票据里的 `CIC = HMAC-SHA256(cicKey, TCD).hexdigest()`
 * （见 backend/xpd.py）。把它搬进浏览器后，签发 xpd 就完全不需要后端了。
 *
 * 正确性：crypto-test.mjs 里对着 Node crypto 逐字节比，并且最终拿
 * `tests/fixtures.mjs` 里 Python 生成的那份 xpd 做逐字节验收。
 */
...
export function hmacSha256(key, data) {
  // 复用 sha1.mjs 里的通用 HMAC（块长 64）
  return hmacGeneric(sha256, 64, key, data);
}

function hmacGeneric(hashFn, blockSize, key, data) {
  ...
  const innerHash = hashFn(join(inner, data));
  return hashFn(join(outer, innerHash));
}
